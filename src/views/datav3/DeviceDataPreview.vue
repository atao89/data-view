<template>
  ...
  <div class="device-data-preview">
    ...
    <div class="device-state left-layout">
      <div class="online-rate">
        ...
        <div class="footer">
          <ul>
            <li
              v-for="item in doorStates"
              :key="item.id"
              :class="[
                { 'd-online': item.id === 'd-online' },
                { 'd-off': item.id === 'd-off' },
              ]"
            >
    ...
    <div class="data-proportion right-layout">
      <div
        class="real-time-Warning"
        :style="{ height: $parent.fullscreen ? height : '315px' }"
      >
      ...
      <div
        ...
        :style="{ height: $parent.fullscreen ? height : '250px' }"
      >
  ...
  methods: {
      ...
      this.doorStates.push(
        {
          ...
          valueArr: this.$parent
            .formatter(deviceStateData.data.onLineDoorControl)
            .split(""),
        },
        {
          id: "d-off",
          name: "离线",
          value: this.$parent.formatter(
            deviceStateData.data.offLineDoorControl
          ),
          valueArr: this.$parent
            .formatter(deviceStateData.data.offLineDoorControl)
            .split(""),
        }
      );
    ...
    initDeviceConfig() {
      const rowNum = Math.floor(
        (this.$refs.scrollTable.offsetHeight - 50) / 42
      );
      ...
      deviceSortData.data.cityDoorControlVOList.forEach((t, index) => {
        ...
        deviceConfigData.push([
          ...
          `<span style="color:rgba(255, 255, 255, 0.7);font-size: 13px">${t.cityName}</span>`,
    ...
    initDeviceWarning() {
      ...
      deviceWarningData.data.realTimeWarningVOList &&
        deviceWarningData.data.realTimeWarningVOList.length &&
        deviceWarningData.data.realTimeWarningVOList.length > 0 &&
        deviceWarningData.data.realTimeWarningVOList.forEach((t, index) => {
          if (index < 15) {
            deviceWarningConfigData.push([
              `<span style="color: #DA3924">设备离线</span>`,
              `<span style="color:#4A90E2;">${t.communtiyName}</span>`,
              `<span style="color:#ccc;">${t.doorControlName} </span>`,
              `<span style="color:#ccc;">${t.realTime} </span>`,
            ]);
          }
        });
    ...
    initDeviceProportionType() {
      ...
      let optionData = [
        {
          value: deviceTypeData.data.iotDoorControlCount || 0,
          name: "互联网门禁",
        },
        { value: deviceTypeData.data.faceCount || 0, name: "人脸设备" },
        {
          value: deviceTypeData.data.faceDoorControlCount || 0,
          name: "人脸门禁机",
        },
        {
          value: deviceTypeData.data.faceDoorControlByadvertiseCount || 0,
          name: "人脸门禁广告机",
        },
        {
          value: deviceTypeData.data.offlineByadvertiseCount || 0,
          name: "线下广告&门禁机",
        },
        {
          value: deviceTypeData.data.offlineByadvertiseDoorCount,
          name: "人脸广告门",
        },
      ];
      let myChart = this.$echarts.init(
        document.getElementById("proportionType")
      );
      let option = {
        tooltip: {
          trigger: "item",
          ...
          formatter: (params) => {
            return `<div>${params.seriesName} <br> ${
              params.data.name
            }：${this.$parent.formatter(params.data.value)} (${
              params.percent
            }%)</div>`;
          },
        },
      ...
      this.$LoopShowTooltip(myChart, option, {
        loopSeries: true,
        interval: 4000,
      });
    },
    ...
    initDeviceModelType() {
      ...
      let optionData = [
        {
          value: deviceAdvertiseModelData.data.qlg2020R8 || 0,
          name: "QLG2020-R8",
        },
        {
          value: deviceAdvertiseModelData.data.qlg19215 || 0,
          name: "QLG19-215",
        },
        {
          value: deviceAdvertiseModelData.data.qlg19c215 || 0,
          name: "QLG19-C215",
        },
        { value: deviceAdvertiseModelData.data.qLL1932 || 0, name: "QLL19-32" },
        {
          value: deviceAdvertiseModelData.data.gll19c32 || 0,
          name: "GLL19-C32",
        },
